...
public partial class _RequestForm : System.Web.UI.Page
{
    ...
    protected void gvTypes_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        else if (e.CommandName == "save")
        {
            ...
            else
            {
                ...
                {
                    ct.ErrorMessage = "Please Select File";

                }
                else if (fileuploader.PostedFile.ContentLength >= 30000000)
                {
                    ct.ErrorMessage = "File must be less then 3mb";
                }
        ...
        switch (e.Row.RowType)
        {
            case DataControlRowType.DataRow:
                ...
                if (myDataRowView["IsRevisedByRegistrar"].ToString().ToLower() == "true")
                {
                    ...
                    if (myDataRowView["canuploadfile"].ToString().ToLower() == "true")
                    {
    ...
    protected void FileUploadCustomValidator_ServerValidate(object source, ServerValidateEventArgs args)
    {
        ...
        {
            args.IsValid = false;
            cv.ErrorMessage = "Please Select File";
            return;
        }
        if (fl.PostedFile.ContentLength > 2000000)
        {
            args.IsValid = false;
            cv.ErrorMessage = "File must be less then 3mb";
        }
