...
public partial class _DBRegistrar : System.Web.UI.Page
{
    ...
    protected void gvTypes_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        else if (e.CommandName == "downloadPic")
        {
    ...
    {
        using (con = new SqlConnection(ConfigurationManager.ConnectionStrings["DashBoardConnectionString"].ConnectionString))
        {
            using (cmd = new SqlCommand("GetPersonalPhoto", con))
            {
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            {
                string filePath = Path.Combine(@"C:\Sportal\DashBoard\Photo_Path", dt.Rows[i]["Personal_Photo"].ToString());
                ...
                if (file.Exists)
                {
                    ...
                    string contentType = GetMimeType(file.Extension);
    ...
    //        for (int i = 0; i < dt.Rows.Count; i++)
    //        {
    //            filePath = System.IO.Path.Combine(Server.MapPath("C:/Sportal/DashBoard/Photo_Path"), dt.Rows[i]["Personal_Photo"].ToString());
    //            FileInfo file = new FileInfo(filePath);
    //            if (file.Exists)
    //            {
    //                Response.Clear();
    ...
    protected void gvForms_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        switch (e.Row.RowType)
        {
            case DataControlRowType.DataRow:
                ...
                //downloadddddddddddddd
                ////    if (int.Parse(myDataRowView["DocumentCount"].ToString()) > 0)
                //    {
                //        LinkButton download = (LinkButton)e.Row.FindControl("btndownloadPic");
                //        if (download != null)
                //        {
