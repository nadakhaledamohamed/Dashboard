...
public partial class Finance_Request : System.Web.UI.Page
{
    ...
    private void FillGv()
    {
        using (con = new SqlConnection(ConfigurationManager.ConnectionStrings["FormRequestConnectionString"].ConnectionString))
        {
            using (cmd = new SqlCommand("GetRequestDetailsForEmployess", con))
            {
                ...
                cmd.Parameters.AddWithValue("@isSchool", true);
    ...
    private void FillStudents()
    {
        using (con = new SqlConnection(ConfigurationManager.ConnectionStrings["FormRequestConnectionString"].ConnectionString))
        {
            ...
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@SchoolId", ddschool.SelectedValue);
                adr = new SqlDataAdapter(cmd);
                dt = new DataTable();
                adr.Fill(dt);
            }
    ...
    private void FillDD()
    {
        ...
        using (con = new SqlConnection(ConfigurationManager.ConnectionStrings["FormRequestConnectionString"].ConnectionString))
        {
            ...
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@availablForSchool", true);
                adr = new SqlDataAdapter(cmd);
                dt = new DataTable();
                adr.Fill(dt);
            }
    ...
    protected void FillParameters()
    {
        using (con = new SqlConnection(ConfigurationManager.ConnectionStrings["FormRequestConnectionString"].ConnectionString))
        {
            using (cmd = new SqlCommand("GetSchoolMemberData", con))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                if(dt.Rows.Count > 0)
                {
                    ...
                    ddschool.SelectedValue = dt.Rows[0]["SchoolId"].ToString();

                }
                ddschool.Enabled = false;
            }
        }
